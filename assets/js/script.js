...
const totalDePerguntas = questoes.length;
...
for (const item of questoes) {
	...
	for (let resposta of item.respostas) {
		...
		const respostaSpan = dt.querySelector('span');
		const inputRadio = dt.querySelector('input');

		respostaSpan.textContent = resposta;
		respostaSpan.addEventListener('click', () => {
			inputRadio.checked = true;
		});

		inputRadio.setAttribute('name', 'pergunta-' + questoes.indexOf(item));
		inputRadio.value = item.respostas.indexOf(resposta);
...
enviarButton.addEventListener('click', () => {
	...
	respostasSelecionadas.forEach((resposta) => {
		...
		const pergunta = questoes[perguntaIndex];
